...
class VoiceListener():
    ...
    def decode(self, result):
        ...
        return Move(self.board.pieceAtPosition(C(self.convert(result[0]), self.convert(result[1]))), C(self.convert(result[3]), self.convert(result[4])), self.board.pieceAtPosition(C(self.convert(result[3]), self.convert(result[4]))))
    ...
    def check(self, piecesFound, result):
        ...
        if len(pieces) == 1:
            m = Move(pieces[0], C(self.convert(result[2]), self.convert(result[3])), self.board.pieceAtPosition(C(self.convert(result[2]), self.convert(result[3]))))
            return m
        return None, None
