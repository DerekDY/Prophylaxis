...
class LEDMatrix():
    ...
    def __init__(self):
        ...
        self.smallFont = "/home/pi/Prophylaxis/rpi-rgb-led-matrix/fonts/4x6.bdf"
        self.normFont = "/home/pi/Prophylaxis/rpi-rgb-led-matrix/fonts/5x8.bdf"
        self.bigFont = "/home/pi/Prophylaxis/rpi-rgb-led-matrix/fonts/6x10.bdf"
    ...
    def draw_box(self,x,y, color):
        ...
        self.set_pixel(x + 1, y + 3, color)
        self.set_pixel(x + 2, y + 3, color)
        self.set_pixel(x + 3, y + 3, color)
        self.set_pixel(x + 3, y + 2, color)
        self.set_pixel(x + 3, y + 1, color)
    ...
    def box_circle(self,x,y,sleepTime,color):
        x2 = x + 4
        y2 = y + 4
        self.draw_box(x,y,color)
        self.draw_box(x2,y2,color)
        mProcess = Process(target=self.refresh, args=())
        mProcess.start()
        time.sleep(sleepTime)

        self.draw_box(x,y,8)
        self.draw_box(x+1,y,color)
        self.draw_box(x2,y2,8)
        self.draw_box(x2-1,y2,color)
        mProcess2 = Process(target=self.refresh, args=())
        mProcess.terminate() 
        mProcess2.start()
        time.sleep(sleepTime) 

        self.draw_box(x+1,y,8)
        self.draw_box(x+2,y,color)
        self.draw_box(x2-1,y2,8)
        self.draw_box(x2-2,y2,color)
        mProcess = Process(target=self.refresh, args=())
        mProcess2.terminate()
        mProcess.start()
        time.sleep(sleepTime)

        self.draw_box(x+2,y,8)
        self.draw_box(x+3,y,color)
        self.draw_box(x2-2,y2,8)
        self.draw_box(x2-3,y2,color)
        mProcess2 = Process(target=self.refresh, args=())
        mProcess.terminate() 
        mProcess2.start()
        time.sleep(sleepTime) 

        self.draw_box(x+3,y,8)
        self.draw_box(x+4,y,color)
        self.draw_box(x2-3,y2,8)
        self.draw_box(x2-4,y2,color)
        mProcess = Process(target=self.refresh, args=())
        mProcess2.terminate()
        mProcess.start()
        time.sleep(sleepTime)

        self.draw_box(x+4,y,8)
        self.draw_box(x+4,y+1,color)
        self.draw_box(x2-4,y2,8)
        self.draw_box(x2-4,y2-1,color)
        mProcess2 = Process(target=self.refresh, args=())
        mProcess.terminate() 
        mProcess2.start()
        time.sleep(sleepTime) 

        self.draw_box(x+4,y+1,8)
        self.draw_box(x+4,y+2,color)
        self.draw_box(x2-4,y2-1,8)
        self.draw_box(x2-4,y2-2,color)
        mProcess = Process(target=self.refresh, args=())
        mProcess2.terminate()
        mProcess.start()
        time.sleep(sleepTime)

        self.draw_box(x+4,y+2,8)
        self.draw_box(x+4,y+3,color)
        self.draw_box(x2-4,y2-2,8)
        self.draw_box(x2-4,y2-3,color)
        mProcess2 = Process(target=self.refresh, args=())
        mProcess.terminate() 
        mProcess2.start()
        time.sleep(sleepTime) 

        self.draw_box(x+4,y+3,8)
        self.draw_box(x+4,y+4,color)
        self.draw_box(x2-4,y2-3,8)
        self.draw_box(x2-4,y2-4,color)
        mProcess = Process(target=self.refresh, args=())
        mProcess2.terminate()
        mProcess.start()
        time.sleep(sleepTime)

        self.draw_box(x+4,y+4,8)
        self.draw_box(x+3,y+4,color)
        self.draw_box(x2-4,y2-4,8)
        self.draw_box(x2-3,y2-4,color)
        mProcess2 = Process(target=self.refresh, args=())
        mProcess.terminate() 
        mProcess2.start()
        time.sleep(sleepTime)

        self.draw_box(x+3,y+4,8)
        self.draw_box(x+2,y+4,color)
        self.draw_box(x2-3,y2-4,8)
        self.draw_box(x2-2,y2-4,color)
        mProcess = Process(target=self.refresh, args=())
        mProcess2.terminate()
        mProcess.start()
        time.sleep(sleepTime)

        self.draw_box(x+2,y+4,8)
        self.draw_box(x+1,y+4,color)
        self.draw_box(x2-2,y2-4,8)
        self.draw_box(x2-1,y2-4,color)
        mProcess2 = Process(target=self.refresh, args=())
        mProcess.terminate() 
        mProcess2.start()
        time.sleep(sleepTime)

        self.draw_box(x+1,y+4,8)
        self.draw_box(x,y+4,color)
        self.draw_box(x2-1,y2-4,8)
        self.draw_box(x2,y2-4,color)
        mProcess = Process(target=self.refresh, args=())
        mProcess2.terminate()
        mProcess.start()
        time.sleep(sleepTime)

        self.draw_box(x,y+4,8)
        self.draw_box(x,y+3,color)
        self.draw_box(x2,y2-4,8)
        self.draw_box(x2,y2-3,color)
        mProcess2 = Process(target=self.refresh, args=())
        mProcess.terminate() 
        mProcess2.start()
        time.sleep(sleepTime)

        self.draw_box(x,y+3,8)
        self.draw_box(x,y+2,color)
        self.draw_box(x2,y2-3,8)
        self.draw_box(x2,y2-2,color)
        mProcess = Process(target=self.refresh, args=())
        mProcess2.terminate()
        mProcess.start()
        time.sleep(sleepTime)

        self.draw_box(x,y+2,8)
        self.draw_box(x,y+1,color)
        self.draw_box(x2,y2-2,8)
        self.draw_box(x2,y2-1,color)
        mProcess2 = Process(target=self.refresh, args=())
        mProcess.terminate() 
        mProcess2.start()
        time.sleep(sleepTime)
        ...
        self.draw_box(x,y+1,8)
        self.draw_box(x,y,color)
        self.draw_box(x2,y2-1,8)
        self.draw_box(x2,y2,color)
        mProcess = Process(target=self.refresh, args=())
        mProcess2.terminate()
        mProcess.start()
